...
//      metadata.json (`foreign_session_id`), so `cerebro --resume <id>` can
...
export const CerebroSessionBinding = async () => {
  ...
  const recordSid = (sid) => {
    ...
    recordedSid = sid
    const m = readMeta()
    if (m.foreign_session_id !== sid) m.foreign_session_id = sid
    // Legacy field from an older single-backend release; clear it so the
    // canonical `foreign_session_id` is the single source of truth.
    if (m.opencode_session_id !== undefined) delete m.opencode_session_id
